...
def trn():
    ...
    try:
        ...
        sql = """
            WITH base AS (
                SELECT
                    TB0008_CD_CONVENIADO AS Posto, TB0153_CD_NSU AS NSU,
                    SUBSTR(TB0153_CD_NSU, LENGTH(TB0008_CD_CONVENIADO) + 1, 1) AS Pista,
                    TO_CHAR(TB0153_DT_TRANSACAO, 'DD/MM/YYYY HH:MM') AS DataHora,
                    ROW_NUMBER() OVER (
                        PARTITION BY TB0008_CD_CONVENIADO,
                                    SUBSTR(TB0153_CD_NSU,
                                    LENGTH(TB0008_CD_CONVENIADO) + 1, 1)
                        ORDER BY TB0153_DT_TRANSACAO DESC
                    ) AS rn
                FROM CADMO.TB0153_TRANSACAOCONVENIADO
                WHERE TB0153_DT_TRANSACAO >= TRUNC(SYSDATE - 400)
                AND TB0153_CD_NSU NOT LIKE '90%'
            )
            SELECT Posto, Pista, NSU, DataHora FROM base WHERE rn = 1 ORDER BY Posto, Pista
        """
        ...
        log_info(f"O Select retornou {len(df)} Transaçoes Pista!")
        ...
        df.columns = ["CREDENCIADO", "PISTA", "NSU", "DATA"]

        # Caminho do arquivo
        caminho_csv = os.path.join(PASTA_DESTINO, "trn_pista.csv")
        ...
        log_error(f"Erro ao processar transacoes_pista: {str(e)}")
        return False
...
def gatilho_trn_pista():
    ...
    proxima_execucao = datetime.strptime(
        controle["proxima_execucao_trn_pista"],
        "%Y-%m-%d %H:%M:%S"
    )

    if agora >= proxima_execucao:

        log_info("Executando rotina TRN Pista...")
        executar = True
    ...
    if executar:
        try:
            ...
            log_error(f"Erro ao executar rotina TRN Pista: {str(e)}")
            return False
